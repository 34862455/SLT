...
# Performs a spatial convolution → (1 × 3 × 3) kernel.
# Captures spatial features without modifying the temporal dimension
# Operates independently across time
...
# Performs a temporal convolution → (3 × 1 × 1) kernel
# Captures motion information across time
# Operates independently across spatial dimensions
def conv_T(in_planes, out_planes, stride=1, padding=1):
    # conv T is 3x1x1
    return nn.Conv3d(in_planes, out_planes, kernel_size=(3, 1, 1), stride=1,
                     padding=padding, bias=False)

# Downsamples input features using avg_pool3d.
# If necessary, adds zero-padding to match output size
# Preserves the number of time steps while reducing spatial dimensions
...
# Defines a ResNet-style bottleneck block
# Uses a combination of 2D & 3D convolutions
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, n_s=0, depth_3d=47, ST_struc=('A', 'B', 'C')):
        ...
        if n_s < self.depth_3d: #3D conv
            ...
            if n_s == self.depth_3d:
                ...
                stride_p = 1
            # First 1x1x1 pointwise 3D convolution reduces feature map depth
        ...
        if n_s < self.depth_3d:
            # Final 1x1x1 convolution restores feature map depth
    ...
    # different ways of combining spatial & temporal convolutions
    # Sequential S → T processing
    def ST_A(self, x):
        x = self.conv2(x)
        x = self.bn2(x)
        x = self.relu(x)
    ...
    # Parallel S + T processing (residual connection)
    def ST_B(self, x):
        tmp_x = self.conv2(x)
...
# Defines full P3D network
# Uses 4 ResNet-style layers (same as ResNet-50)
class P3D(nn.Module):
    def __init__(self, block, layers, input_channel=3,
        ...
        # Wide receptive field for spatial extraction
    ...
    # Converts final features into classification output
    def forward(self, x):
